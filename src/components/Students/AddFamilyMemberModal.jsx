...
import { X, UserPlus, GraduationCap, Check, ArrowLeft } from 'lucide-react';
 ...
 * card. A family's account so far only grew by importing a new student
 ...
 *
 * Opens on a chooser rather than dropping straight into the guardian form —
 * the first version buried "add a student" as a link at the bottom of that
 * form, and it went unnoticed.
 */
const AddFamilyMemberModal = ({ family, onClose, onSaved, onAddStudent }) => {
  const toast = useToast();
  const [mode, setMode] = useState(null); // null | 'guardian'
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content afm-modal" onClick={e => e.stopPropagation()}>
        <div className="asm-header">
          {mode === 'guardian' ? (
            <button className="icon-btn" onClick={() => setMode(null)} aria-label="Back">
              <ArrowLeft size={20} />
            </button>
          ) : (
            <div />
        ...
        {mode === null && (
          <div className="asm-body">
            <div className="afm-choice-grid">
              <button type="button" className="afm-choice-card" onClick={() => setMode('guardian')}>
                <UserPlus size={28} />
                <strong>Add a Guardian</strong>
                <span>A second parent or guardian who needs their own login.</span>
              </button>
              <button
                type="button"
                className="afm-choice-card"
                onClick={() => { onAddStudent(family.id); onClose(); }}
              >
                <GraduationCap size={28} />
                <strong>Add a Student</strong>
                <span>Enroll another child under this same family account.</span>
              </button>
            </div>
          </div>
        )}
